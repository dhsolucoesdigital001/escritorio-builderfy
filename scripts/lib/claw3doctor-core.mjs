...
export const isCustomRuntimeAdapter = (adapterType) => {
  ...
  return (
    normalized === "custom" ||
    normalized === "local" ||
    normalized === "escritorio-builderfy"
  );
};
...
export const resolveRuntimeContext = ({
  ...
  const adapterType = normalizeAdapterType(
    gateway?.adapterType ??
      ...
      env.ESCRITORIO-BUILDERFY_GATEWAY_ADAPTER_TYPE,
    "openclaw",
  );
...
export const buildGatewayFailureActions = ({
  ...
  if (normalized.includes("econnrefused") || normalized.includes("timed out")) {
    actions.push(
      "Verify the backend is actually listening on the configured host and port before retrying from Escritorio Builderfy.",
    );
  }
...
export const formatDoctorReport = ({
  ...
  const lines = [];
  lines.push("==================================================");
  lines.push(`Escritorio Builderfyoctor ${formatStatusBadge(summary)}`);
  lines.push("==================================================");
  lines.push("");
