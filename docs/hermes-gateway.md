...
Escritorio Builderfy can run against Hermes by using the bundled adapter in
...
Escritorio Builderfy-compatible WebSocket adapter.
...
The frontend keeps using the Escritorio Builderfy gateway protocol. The Hermes adapter
...
### 3. Start Escritorio Builderfy and the adapter
...
In the connect screen, select `Hermes backend`. Escritorio Builderfy will persist that
...
The adapter currently supports the Escritorio Builderfy surfaces needed for normal
office use:
...
Escritorio Builderfy gateway contract today and is ready for upstream testing now.
...
That starts a bundled mock gateway for a no-framework Escritorio Builderfy demo.

## Using OpenClaw instead

If you want the OpenClaw path, do not run the Hermes adapter. Start
OpenClaw and point Escritorio Builderfy at that gateway instead.
