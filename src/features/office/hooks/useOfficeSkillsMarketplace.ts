  ...
  const handleInstallPackagedSkill = useCallback(
    async (skillKey: string) => {
      ...
      await runSkillMutation({
        ...
        successMessage: `Successfully installed ${packagedSkill.name.trim()} in the selected workspace. Enable it for the agent from the ESCRITORIO-BUILDERFY tab.`,
  ...
  const handleInstallPackagedSkillAndEnable = useCallback(
    async (params: {
      ...
      try {
        ...
        params.onProgress?.({
          percent: 92,
          message: "Refreshing skill state in Escritorio Builderfy.",
        });
