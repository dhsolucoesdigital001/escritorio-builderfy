...
Define a clean `custom` runtime provider class for Escritorio Builderfy.

This provider should let external orchestration stacks integrate with Escritorio Builderfy through a stable seam without requiring:
...
- Escritorio Builderfy probes `GET /health`, `GET /state`, and `GET /registry`
- chat uses a direct HTTP path to `POST /v1/chat/completions`
- browser traffic is proxied through Escritorio Builderfy's same-origin
...
Not every useful runtime should have to become a named built-in provider in upstream Escritorio Builderfy.
...
Escritorio Builderfy’s runtime abstraction already points toward multiple providers:
...
It should not require upstream Escritorio Builderfy to know:
...
This gives Escritorio Builderfy enough identity for UI without hardcoding a brand into the provider class.
...
- upstream Escritorio Builderfy gets `custom`
...
This keeps Escritorio Builderfy stable even if the custom stack has richer private event semantics internally.
...
The custom provider is the right place for richer internal state to enter Escritorio Builderfy.
...
The custom provider should be able to support office systems without Escritorio Builderfy needing to know the backend’s internals.
...
- make no runtime-specific assumptions in core Escritorio Builderfy

Actual runtime adapters can be implemented separately against that seam.

## Out of Scope For V1

- embedding proprietary internal orchestration logic in Escritorio Builderfy core
...
- upstream Escritorio Builderfy gains a clean extension seam
...
It gives Escritorio Builderfy extensibility, gives Vera a clean path in, and avoids hardwiring a personal stack directly into the core product model.
