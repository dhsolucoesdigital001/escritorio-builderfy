...
This document exists to onboard coding agents quickly when debugging chat issues in Escritorio Builderfy.
...
- `~/.openclaw/escritorio-builderfy/settings.json` (see `README.md` for the canonical location)
...
- The Studio server resolves an upstream token from `escritorio-builderfy/settings.json`, and if it is missing it may fall back to the local OpenClaw config in `openclaw.json` (token + port). This behavior exists in both the WS proxy path (`server/studio-settings.js`) and the `/api/studio` storage layer (`src/lib/studio/settings-store.ts`) and they should remain consistent.
