# Skills in OpenClaw + Escritorio Builderfy

This document explains skills from first principles, how they work in the OpenClaw runtime, and how Escritorio Builderfy currently exposes them in UX.
...
## 10) Escritorio Builderfy UX (current behavior)
...
Sorted by most recent creation time in `escritorio-builderfy/.agent/done`, the latest items are mostly bugfix exec plans (streaming, proxy auth, stale config, cron rollback, etc.).
