# Escritorio Builderfy — A 3D Workspace for AI Agents

<p align="center">
    <img src="assets/branding/escritorio-builderfy-hero.png" alt="Escritorio Builderfy" width="700">
</p>
...
<p align="center">
  <a href="https://github.com/iamlukethedev/Escritorio Builderfy/actions/workflows/docker-publish.yml?query=branch%3Amain"><img src="https://img.shields.io/github/actions/workflow/status/iamlukethedev/Escritorio Builderfy/docker-publish.yml?branch=main&style=for-the-badge" alt="CI status"></a>
  <a href="https://github.com/iamlukethedev/Escritorio Builderfy/releases"><img src="https://img.shields.io/github/v/release/iamlukethedev/Escritorio Builderfy?include_prereleases&style=for-the-badge" alt="GitHub release"></a>
...
**Escritorio Builderfy** is a _3D virtual office for AI agents_ you run on your own infrastructure.
...
[Website](https://www.escritorio-builderfy.ai/) · [Vision](VISION.md) · [Architecture](ARCHITECTURE.md) · [Tutorial](TUTORIAL.md) · [Getting Started](#quick-start) · [Runtime Profiles](docs/runtime-profiles.md) · [Multi-Agent Beta](docs/multi-agent-beta.md) · [Contributing](CONTRIBUTING.md) · [Security](SECURITY.md) · [Discord](https://discord.gg/UzwNn5Zkhr)

> **Unofficial project.** Escritorio Builderfy is an independent, community-driven project and is not affiliated with, endorsed by, or maintained by the OpenClaw team. OpenClaw is a separate project, and this repository is not the official OpenClaw repository.

Built and maintained by **LukeTheDev**. Follow on X: [@iamlukethedev](https://x.com/iamlukethedev).

## What you can do with Escritorio Builderfy
...
## What Escritorio Builderfy Is

Escritorio Builderfy is the visualization and interaction layer.
...
This repository does not build the upstream runtimes themselves. It is the frontend, Studio, and adapter/proxy layer that connects to a runtime speaking the Escritorio Builderfy gateway protocol.

## Why It Exists

AI systems are becoming more capable, but their work is still usually hidden behind logs, terminal output, and dashboards.

Escritorio Builderfy exists to make agent systems visible:
...
The current app already includes a substantial Escritorio Builderfy surface:
...
- Escritorio Builderfy does not install or build OpenClaw or Hermes for you.
- Before starting Escritorio Builderfy against a real backend, make sure your chosen runtime is already running and that you know the gateway URL and token Studio should use.
- For a no-framework local office demo, run the bundled demo gateway instead.
- If you need a full cross-machine setup guide (OpenClaw + Tailscale + Escritorio Builderfy), follow [`TUTORIAL.md`](TUTORIAL.md).

Run from source:

```bash
git clone <your-public-repo-url> escritorio-builderfy
...
Studio now also persists the selected backend mode (`OpenClaw`, `Hermes`, `Demo`, `Local`, `Escritorio Builderfy`, or `Custom`) and
...
HTTP runtime seam, start your runtime first, then start Escritorio Builderfy:

```bash
npm run dev
```

Then open `http://localhost:3000`, choose `Local runtime`, `Escritorio Builderfy runtime`,
...
The browser does not call that runtime directly. Escritorio Builderfy proxies the
...
Escritorio Builderfy uses two separate network hops:
...
3. Open Escritorio Builderfy from the LAN/Tailscale address instead of `localhost`.
...
- Studio settings: `~/.openclaw/escritorio-builderfy/settings.json`
...
- `ESCRITORIO-BUILDERFY_GATEWAY_URL` and `ESCRITORIO-BUILDERFY_GATEWAY_TOKEN` provide a runtime alternative to `NEXT_PUBLIC_GATEWAY_URL` that takes effect on server restart without a rebuild.
- `ESCRITORIO-BUILDERFY_GATEWAY_ADAPTER_TYPE` can pair with `ESCRITORIO-BUILDERFY_GATEWAY_URL` to mark those runtime defaults as `openclaw`, `hermes`, `demo`, `local`, `escritorio-builderfy`, or `custom`.
- If `ESCRITORIO-BUILDERFY_GATEWAY_URL` is not set, Studio can still surface local Hermes or demo adapter defaults from `HERMES_ADAPTER_PORT` / `DEMO_ADAPTER_PORT`.
...
- [`TUTORIAL.md`](TUTORIAL.md): detailed step-by-step setup for OpenClaw + Tailscale + Escritorio Builderfy.
...
- `INVALID_REQUEST` errors mentioning `minProtocol` or `maxProtocol` usually mean the gateway is too old for Escritorio Builderfy protocol v3. Upgrade OpenClaw, use the Hermes adapter, or run `npm run demo-gateway`.
...
1. Keep Escritorio Builderfy running locally on `http://localhost:3000`.
...
- The main Escritorio Builderfy app stays on `localhost`, so your normal local office state and agent state remain intact.
- Spotify redirects to the `ngrok` callback URL.
- The callback page passes the auth code back to the open local Escritorio Builderfy window.
...
For first-time SSH connections, Escritorio Builderfy uses `StrictHostKeyChecking=accept-new` by default so a new host key can be trusted automatically. If you need stricter behavior, set `OPENCLAW_GATEWAY_SSH_STRICT_HOST_KEY_CHECKING=yes`, or set it to `no` only if you explicitly want to skip host key checks.
...
If you use Cursor or another AI-assisted workflow, review the committed project guardrails in [`.cursor/rules/escritorio-builderfy-project-guardrails.mdc`](.cursor/rules/escritorio-builderfy-project-guardrails.mdc).

That rule file captures the shared editing expectations for this repository, including the Escritorio Builderfy-vs-OpenClaw boundary, code placement conventions, office-stack distinctions, and documentation/test update expectations.
