...
Escritorio Builderfy now treats runtime backends as named saved profiles instead of one global URL/token pair.
...
This is the provider-rich path. OpenClaw already knows how to sit in front of many upstream model providers, so Escritorio Builderfy should treat it as a first-class gateway adapter rather than flattening it into `custom`.
...
A Escritorio Builderfy-shaped HTTP runtime profile for stacks that want to keep Escritorio Builderfy transcript and chat conventions while still using the direct runtime seam.
...
The generic HTTP runtime seam when you want to point Escritorio Builderfy at any compatible orchestrator boundary.
...
That means `local`, `escritorio-builderfy`, and `custom` are first-class saved profiles today.
...
The current provider review path should borrow from existing Hermes/OpenClaw wizard flows where possible, but land as Escritorio Builderfy-native adapters instead of hard-coupling Escritorio Builderfy UI state to another project's connector code.
