...
const STORE_DIR = path.join("escritorio-builderfy", "task-manager");
...
const normalizeTaskRecord = (value: unknown): SharedTaskRecord | null => {
  ...
  return {
    ...
    source: isTaskBoardSource(record.source) ? record.source : "escritorio-builderfy_manual",
...
export const upsertSharedTask = (
  ...
  const rawSource = (task.source as TaskBoardSource | undefined) ?? existing?.source ?? "escritorio-builderfy_manual";
  ...
  const next: SharedTaskRecord = {
    ...
    source: isTaskBoardSource(rawSource) ? rawSource : "escritorio-builderfy_manual",
