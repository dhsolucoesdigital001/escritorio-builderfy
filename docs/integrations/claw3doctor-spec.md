# Escritorio Builderfyoctor Spec

> First-pass diagnostics plan for Escritorio Builderfy deployments so users stop chasing the same setup failures manually.

## Goal

Provide a single diagnostics surface for the common "Escritorio Builderfy cannot connect"
...
but focused on Escritorio Builderfy's integration points across providers.

## Primary Outcomes

`escritorio-builderfyoctor` should:
...
### Escritorio Builderfy Settings / Environment
...
- does the endpoint respond like a Escritorio Builderfy-compatible gateway?
...
`escritorio-builderfyoctor` should produce:
...
```text
Escritorio Builderfyoctor: WARN
...
`escritorio-builderfyoctor` should be designed against the runtime-profile model:
...
`escritorio-builderfyoctor` should land before more runtime complexity because it will
make debugging:
...
`escritorio-builderfyoctor` v1 should be considered complete when it provides:
...
`escritorio-builderfyoctor` v2 should also benefit from the separate runtime-profile work:
