...
describe("shared task store", () => {
  ...
  it("recovers gracefully from corrupted JSON on disk", () => {
    ...
    upsertSharedTask({ id: "t-1", title: "Valid task", status: "todo", source: "escritorio-builderfy_manual" });
    ...
    const afterCorrupt = upsertSharedTask({ id: "t-2", title: "After recovery", status: "todo", source: "escritorio-builderfy_manual" });
  ...
  it("performs atomic writes so partial failures don't corrupt the store", () => {
    ...
    upsertSharedTask({ id: "t-1", title: "Safe task", status: "todo", source: "escritorio-builderfy_manual" });
    const storePath = resolveSharedTaskStorePath();
  ...
  it("coerces invalid status and source to defaults", () => {
    ...
    expect(task.source).toBe("escritorio-builderfy_manual");
  });
