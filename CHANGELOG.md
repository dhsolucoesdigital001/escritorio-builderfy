...
This release converges the runtime-profiles, office-systems, escritorio-builderfyoctor, and selective `vera_lane` work into one branch. Backends become named profiles, the office becomes a multi-floor building, remote offices gain server-backed messaging and handoffs, and a new diagnostics CLI makes setup and troubleshooting first-class.

### Added

- Named runtime profiles for `openclaw`, `hermes`, `demo`, `local`, `escritorio-builderfy`, and `custom` backends, each storing its own URL and token in Studio settings instead of a single global pair (`docs/runtime-profiles.md`, `src/lib/runtime/*`).
- Multi-floor office runtime model with one runtime binding per floor and persistent per-floor state, including `lobby`, `openclaw-ground`, `hermes-first`, `local-runtime`, `escritorio-builderfy-runtime`, `custom-second`, `training`, `traders-floor`, and `campus` (`src/lib/office/floors.ts`, `docs/office_sys/multi-floor-runtime-architecture.md`).
...
- `escritorio-builderfyoctor` diagnostics CLI with profile-scoped and `--all-profiles` runs, OpenClaw / Hermes / demo / custom-runtime probes, gateway failure classification, JSON output, and a `npm run doctor` script (`scripts/escritorio-builderfyoctor.mjs`, `scripts/lib/escritorio-builderfyoctor-core.mjs`, `package.json`).
- New product and architecture docs covering runtime profiles, multi-floor architecture, the escritorio-builderfyoctor spec, runtime profile architecture, the refreshed roadmap, multi-agent beta, and the bulletin-board, desk-progression, hierarchy-and-teams, meeting-room-workflow, QA-department, and whiteboard specs (`docs/`).

### Changed

- Runtime provider selection is now profile-aware across `openclaw`, `hermes`, `demo`, `local`, `escritorio-builderfy`, and `custom` instead of collapsing into one generic path (`src/lib/runtime/createRuntimeProvider.ts`, `src/lib/runtime/{openclaw,hermes,demo,custom}/provider.ts`).
...
- Added unit coverage for `escritorio-builderfyoctor`, office floors, floor roster, runtime connection, gateway connection, office floor runtime persistence, agent fleet hydration derivation, and studio settings coordinator behavior (`tests/unit/`).
...
- Expanded `README.md` to describe `Local` and `Escritorio Builderfy` runtime modes and persistent backend profile configuration, including the additional `local` and `escritorio-builderfy` values for `ESCRITORIO-BUILDERFY_GATEWAY_ADAPTER_TYPE`.
...
This release expands Escritorio Builderfy from a single-office viewer into a more complete AI workplace, with guided setup, richer agent operations, remote office support, and stronger security hardening.
...
- Added an Agent Bus integration guide for visualizing AI sessions in Escritorio Builderfy.
...
- Initial public Escritorio Builderfy project documentation, including `README.md`, `VISION.md`, and `ARCHITECTURE.md`.
