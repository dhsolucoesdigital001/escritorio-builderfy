...
This document explains the current multi-agent beta in Escritorio Builderfy: what it does, how the two connection modes work, and how to connect a second office.

## What This Beta Does

Escritorio Builderfy can render a second office inside the same 3D scene so you can visualize agents from another machine.
...
- **Local office**: the Escritorio Builderfy instance you are currently using;
- **Remote office**: another Escritorio Builderfy instance or another OpenClaw gateway you want to visualize.

The remote office can be connected in one of two ways:

1. **Remote Escritorio Builderfy presence endpoint**.
...
Use this when the other machine is also running Escritorio Builderfy.

How it works:

- your local Escritorio Builderfy server polls the remote Escritorio Builderfy `presence` endpoint;
...
This mode is best when you want the remote side to feel like another full Escritorio Builderfy office.

### 2. Remote OpenClaw Gateway

Use this when the other machine only runs OpenClaw and does not run Escritorio Builderfy.

How it works:

- the browser connects directly to the remote gateway;
- Escritorio Builderfy derives a read-only presence snapshot from gateway data such as `agents.list`, `status`, and `sessions.preview`;
- because there is no remote Escritorio Builderfy layout endpoint, the second office uses a fallback office visualization.
...
If you paste an `http://` or `https://` URL into gateway mode, Escritorio Builderfy normalizes it to `ws://` or `wss://` before connecting.

This mode is best when you want remote agent visibility without requiring a second Escritorio Builderfy deployment.
...
- is designed to avoid exposing remote files or tool output in the Escritorio Builderfy UI.
...
Under the hood, Escritorio Builderfy now uses a shared runtime contract for:

- `agents.message`
- `agents.handoff`

OpenClaw, Hermes, Demo, and direct custom/local/escritorio-builderfy runtime profiles can all target the same message/handoff seam. Provider-native adapters such as Anthropic or Claude Code are still a follow-up slice.
...
- your local Escritorio Builderfy is already working with your local OpenClaw gateway;
...
1. Start your local Escritorio Builderfy instance.
...
### Setup For `Remote Escritorio Builderfy presence endpoint`

Use:

- `Source type`: `Remote Escritorio Builderfy presence endpoint`.
- `Presence URL`: the remote `/api/office/presence` URL.
- `Optional token`: only if that remote Escritorio Builderfy endpoint is protected.
...
- if the remote layout snapshot is unavailable, Escritorio Builderfy falls back to a default/fallback office rendering for the remote side.
...
- the office shell is a fallback visualization, not a true remote layout clone from another Escritorio Builderfy instance.

## Recommended Network Patterns

### Same private network

Use a reachable private IP or local hostname for the remote Escritorio Builderfy endpoint or OpenClaw gateway.
...
- remote Escritorio Builderfy endpoint over `https://<machine>.ts.net/api/office/presence`;
...
That usually means the other machine has Escritorio Builderfy presence available but not a layout snapshot yet. The beta should still render a fallback remote office.
...
- Gateway mode derives presence from gateway snapshots rather than a real remote Escritorio Builderfy layout.
...
Use `Remote Escritorio Builderfy presence endpoint` when the other side runs Escritorio Builderfy and you want the most complete office visualization.
