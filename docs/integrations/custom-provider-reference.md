...
> Reference implementation guide for plugging a non-OpenClaw, non-Hermes runtime into Escritorio Builderfy through the upstream-safe `custom` provider seam.

## Goal

Show how a custom orchestration stack should plug into Escritorio Builderfy without requiring:
...
- Escritorio Builderfy sees `custom`
- the external runtime stays implementation-specific
- Escritorio Builderfy core remains generic
...
A custom runtime should sit at the Escritorio Builderfy boundary as an orchestrator-backed service.

That means:

- Escritorio Builderfy should talk to one stable runtime boundary
...
The `custom` provider exists so those stacks can integrate cleanly without forcing Escritorio Builderfy core to absorb stack-specific assumptions.

## Recommended Boundary

Escritorio Builderfy should integrate with the custom runtime's orchestrator or gateway layer, not with its individual workers.
...
This gives the UI enough identity without forcing upstream Escritorio Builderfy to branch on every runtime brand.
...
Do not claim support just because the backend can theoretically do something. Claim support only where the provider exposes a stable Escritorio Builderfy-facing behavior.

## Agent Mapping

Escritorio Builderfy agents should not be modeled as one fixed backend process each unless the runtime truly behaves that way.
...
Escritorio Builderfy sessions should map to runtime conversations or execution threads, not to whichever backend storage structure happens to exist internally.
...
The provider should translate runtime activity into the same normalized Escritorio Builderfy runtime events used elsewhere.
...
The custom provider is the right place to map richer internal runtime signals into Escritorio Builderfy's public office state model.
...
Escritorio Builderfy only needs the resulting office-safe state and maybe a public reason label.

## Relationship To Office Systems

The custom provider should support office systems without Escritorio Builderfy needing to know the backend's internals.
...
3. Map runtime roles or routes into Escritorio Builderfy agent identities.
...
This is enough to make a custom runtime useful in Escritorio Builderfy without overexposing internals.
...
- runtime-specific branches throughout Escritorio Builderfy core
