...
This file is the practical code map for Escritorio Builderfy contributors.
...
Escritorio Builderfy is the UI and local Studio/proxy layer around an existing OpenClaw Gateway.

- OpenClaw owns agent execution, sessions, tools, config, and runtime events.
- Escritorio Builderfy owns visualization, local Studio settings, UI workflows, office rendering, and the same-origin WebSocket/API bridge.
...
Sometimes Escritorio Builderfy behavior depends on the upstream event contract or session behavior. In those cases:
...
3. Apply changes in Escritorio Builderfy unless the user explicitly asked for upstream OpenClaw work.
