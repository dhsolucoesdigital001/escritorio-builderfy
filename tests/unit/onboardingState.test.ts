...
describe("useOnboardingState", () => {
  afterEach(() => {
    ...
    try {
      window.localStorage.removeItem("escritorio-builderfy:onboarding:completed");
    } catch {
      // noop
    }
  });
  ...
  it("persists completion to localStorage", () => {
    ...
    expect(window.localStorage.getItem("escritorio-builderfy:onboarding:completed")).toBe(
      "true",
    );
  });

  it("reads completion state from localStorage on mount", () => {
    window.localStorage.setItem("escritorio-builderfy:onboarding:completed", "true");
    const { result } = renderHook(() => useOnboardingState());
    expect(result.current.showOnboarding).toBe(false);
  });
  ...
  it("resets onboarding when resetOnboarding is called", () => {
    ...
    expect(result.current.showOnboarding).toBe(true);
    expect(window.localStorage.getItem("escritorio-builderfy:onboarding:completed")).toBeNull();
  });
});
