...
> Visualize AI coding sessions in Escritorio Builderfy's retro office — without OpenClaw.

[Agent Bus](https://github.com/emiliovos/agent-bus) is an open-source event routing system that bridges AI coding agents (Claude Code, Gemini, Codex, etc.) to Escritorio Builderfy. Agents appear in the 3D office, animate when working, and go idle between tasks. Zero inference cost — pure data routing.

## How It Works

```
AI coding session → hook fires → Agent Bus hub → gateway :18789 → Escritorio Builderfy renders in 3D
```

Agent Bus includes an **OpenClaw-compatible gateway** that speaks the same WebSocket protocol Escritorio Builderfy already uses. No Escritorio Builderfy code changes needed — just point `GATEWAY_URL` to the Agent Bus gateway.
...
- Node.js 18+
- Escritorio Builderfy running on `:3000`
...
### Connect Escritorio Builderfy

Point Escritorio Builderfy's gateway URL to Agent Bus:

```bash
# In your Escritorio Builderfy .env or environment:
GATEWAY_URL=ws://localhost:18789
```

Restart Escritorio Builderfy. It will connect to the Agent Bus gateway instead of OpenClaw.
...
Every tool use in Claude Code now appears as agent activity in Escritorio Builderfy.
...
This exposes the hub and Escritorio Builderfy via HTTPS with service token authentication. Agents on remote machines (VPS, other PCs) can send events through the tunnel.
