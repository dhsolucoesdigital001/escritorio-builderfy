# Escritorio Builderfy + OpenClaw + Tailscale Setup Tutorial

This guide is a step-by-step runbook for the most common production-like setup:

- **Machine A** runs **OpenClaw Gateway**.
- **Machine B** runs **Escritorio Builderfy**.
- **Tailscale** connects both machines securely.

If you follow this exactly, people should avoid the most common confusion: **Escritorio Builderfy does not install or run OpenClaw for you.**
...
- **Escritorio Builderfy** is the UI and Studio proxy.
- Escritorio Builderfy connects to an already running OpenClaw Gateway.
- In this tutorial, the Gateway lives on a different machine from Escritorio Builderfy.
...
### Machine B (Escritorio Builderfy host)
...
You will need this token in Escritorio Builderfy:
...
This host is what Escritorio Builderfy will use as `wss://<gateway-host>.<tailnet>.ts.net`.

---

## 5) Install and Run Escritorio Builderfy on Machine B

On **Machine B**:

```bash
git clone https://github.com/iamlukethedev/Escritorio Builderfy.git escritorio-builderfy
...
## 6) Connect Escritorio Builderfy to OpenClaw

In Escritorio Builderfy connection UI:
...
- Use `ws://localhost:18789` only when Gateway is local to the same machine as Escritorio Builderfy or when using an SSH tunnel.
...
After Escritorio Builderfy is running and tries to connect for the first time, approve pending device pairing on **Machine A**:
...
4. Escritorio Builderfy connect UI uses `wss://...ts.net` plus valid token.
5. `openclaw devices approve --latest` has been run after first connect attempt.
6. Escritorio Builderfy UI shows gateway connected and loads agents.
...
### `401` or auth errors from Escritorio Builderfy

- Re-copy token from Machine A:
  - `openclaw config get gateway.auth.token`.
- Confirm Gateway auth mode and token are current.

### Escritorio Builderfy still cannot connect after token is correct
