...
> Forward-looking runtime model for Escritorio Builderfy after the OpenClaw + Hermes adapter work landed on `main`.

## Goal

Escritorio Builderfy should treat runtime connection targets as profiles, not as ad hoc
...
- the UI keeps speaking one Escritorio Builderfy gateway contract
...
- where does Escritorio Builderfy connect?
...
- if nothing else is configured, Escritorio Builderfy prefers `OpenClaw`
...
This avoids making the "Custom Floor" logic one-off and lets Escritorio Builderfy grow
...
- the browser talks one Escritorio Builderfy gateway contract
...
- `feat/escritorio-builderfyoctor`
...
### 2. `feat: escritorio-builderfyoctor`

First pass should check:

- Escritorio Builderfy settings/env
...
Escritorio Builderfy should move to a runtime profile model where:
