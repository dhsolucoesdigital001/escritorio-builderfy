...
const normalizeLocalGatewayDefaults = (value: unknown): StudioGatewaySettings | null => {
  ...
  const adapterType =
    ...
    raw.adapterType === "escritorio-builderfy" ||
...
const normalizeGatewayProfilesPublic = (
  ...
  for (const adapterType of ["openclaw", "hermes", "demo", "local", "escritorio-builderfy", "custom"] as const) {
...
const protocolMismatchHint =
  "This gateway looks too old for Escritorio Builderfy's protocol v3. Upgrade OpenClaw, use the Hermes adapter, or run `npm run demo-gateway` for a no-framework office demo.";
...
const pairingRequiredHint =
  "This gateway is asking for first-time device approval. Run `openclaw devices approve --latest` on the gateway host, then restart Escritorio Builderfy and reconnect from this browser.";

const requiresDeviceIdentityHint =
  "This gateway rejected the client as a control UI without device identity. For remote OpenClaw/Tailscale connections, update to the latest Escritorio Builderfy build and approve the device pairing on the gateway host.";
...
export const useGatewayConnection = (
  ...
  const connect = useCallback(async () => {
    ...
    if (
      ...
      selectedAdapterType === "escritorio-builderfy"
    ) {
